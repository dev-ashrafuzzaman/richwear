...
export default function TypeCreateModal({ isOpen, setIsOpen, onSuccess }) {
  const { request, loading } = useApi();
  // const [isLocation, setLocation] = useState();
  const [isFace, setFace] = useState();
  const {
    register,
    handleSubmit,
  ...
  const onSubmit = async (input) => {
    if (!isFace) {
      ...
      if (!isFace) missing.push("Face");
      return toast.error(`Please select ${missing.join(" and ")}`);
    }

    const data = {
      // location_id: isLocation?.raw?.id,
      face_id: isFace?.raw?.id,
      name: input.name,
      details: input.details,
    };

    await request("/account-types/", "POST", data, {
      retries: 2,
      ...
      footer={
        <div className="flex justify-end gap-2">
          ...
          {/* <button
            ...
            className="px-4 py-2  text-white rounded-md btn-gradient"
          >
      ...
      <div className="flex flex-col gap-4">
        ...
        <div>
          ...
          <SmartSelect
            ...
            onChange={(val) => setFace(val)}
            placeholder="Search Faces from API..."
          />
        </div>
        {/* Name */}
        <div>
